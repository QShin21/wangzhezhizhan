...
- `wzzz_v__ty_ex__chengxiang`：补充称象专用选牌规则并改为“亮出牌/获得”双区域排列，允许正常拖选获得牌；前端与服务端均限制获得牌点数和不大于 13。
- `wzzz_v__kunfen` / `wzzz_v__kunfenEx`：修正魏势力 SP 姜维初始技能为基础“困奋”，移除初始火攻效果；“逢亮”觉醒后再替换为强化“困奋”并获得视为使用【火攻】的后续效果。
- `wzzz_v__v11__xiechan`：修复许褚“裸衣”后进入响应窗口刷新技能按钮时报错的问题；“挟缠”的主动技可用性判断不再访问仅服务端存在的 `player.room`，并限制为出牌阶段可用，拼点目标仍由目标过滤判定。
- `wzzz_v__jishi` / `wzzz_v__ex__chuli` / `wzzz_v__m_ex__qingnang`：参考“述志”的硬性限定处理，为“济世”选择的“除疠”或“青囊”增加本局限一次判定；被选技能发动前检查济世限定标记和本局发动记录，发动后写入已用标记、同步本局使用次数并失效该技能。
- `wzzz_v__m_ex__jianying`：将主动指定花色的阶段限一次改为独立阶段标记，不再读取同名技能的阶段发动次数，避免第一段无次数摸牌效果触发后误占用第二段主动效果。
- `wzzz_v__huoshou`：修复出牌阶段结束弃置所有手牌后视为使用【南蛮入侵】不生效的问题；改为克隆虚拟【南蛮入侵】并通过 `room:useCard` 空目标使用，由引擎按全体锦囊规则自动补齐所有其他合法目标。
- `wzzz_v__huashen`：化身可声明技能池增加统一禁用判断，除技能标签外也按描述开头兜底排除主公技、限定技和觉醒技；同时对化身技能被替换后的实际技能再次复查，避免漏标或替换链使禁用技能进入可选项。
- `wzzz_v__kunfen` / `wzzz_v__kunfenEx`：修正“困奋”回归为始终可选发动，移除基础困奋的锁定技标签与描述；强化困奋同样在结束阶段询问是否发动，并仅在觉醒后保留额外视为使用【火攻】的选择。
- `wzzz_v__ol__yaoming`：修复前端刷新技能按钮时主动技可用性判断访问 `player.room` 导致空值报错的问题；主动阶段选项判断改为兼容客户端 `Fk:currentRoom()`，服务端结算流程保持原逻辑。
- `wzzz_v__m_ex__quanji` / `wzzz_v__zili` / `wzzz_v__m_ex__paiyi`：将钟会“权”的牌堆键统一为客户端可识别、可展开的 `zhonghui_quan`；权计注册派生牌堆并公开放置牌，自立与排异同步读取该牌堆，修复多个“权”仍显示“权1”且点击标签无法查看对应牌的问题。
