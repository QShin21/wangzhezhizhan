...
Fk:loadTranslationTable{
  ...
  ["$wzzz_v__v33__zhanshen2"] = "战神既出，谁与争锋！",
}
...
v33__zhanshen:addEffect(fk.EventPhaseStart, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    local choice = room:askToChoice(player, {
      ...
      prompt = "#wzzz_v__v33__zhanshen-choice",
    })
...
v33__zhanshen:addEffect(fk.DrawNCards, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:getMark("wzzz_v__v33__zhanshen_1") > 0
  end,
  on_use = function(self, event, target, player, data)
    data.n = data.n + 1
  end,
...
v33__zhanshen:addEffect(fk.PreCardUse, {
  ...
  is_delay_effect = true,
  can_trigger = function(self, event, target, player, data)
    return target == player and player:getMark("wzzz_v__v33__zhanshen_2") > 0 and data.card.trueName == "slash"
  end,
  on_use = function(self, event, target, player, data)
...
v33__zhanshen:addEffect(fk.AfterCardTargetDeclared, {
  ...
  is_delay_effect = true,
  can_trigger = function(self, event, target, player, data)
    return target == player and player:getMark("wzzz_v__v33__zhanshen_3") > 0 and data.card.trueName == "slash" and
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local tos = room:askToChoosePlayers(player, {
      ...
      min_num = 1,
      max_num = 1,
      prompt = "#wzzz_v__v33__zhanshen-choose:::"..data.card:toLogString(),
      skill_name = v33__zhanshen.name,
    })
    if #tos > 0 then
