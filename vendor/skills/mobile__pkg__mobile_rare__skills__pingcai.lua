...
Fk:loadTranslationTable{
  ["wzzz_v__pingcai"] = "评才",
  [":wzzz_v__pingcai"] = "出牌阶段限一次，你可以挑选一个宝物，擦拭掉上面的灰尘。如果擦拭成功，你可以根据宝物类型执行对应的效果：<br>"..
  ...
  ["#wzzz_v__pingcai"] = "评才：选择一个宝物擦拭灰尘！",
  ["wzzz_v__pingcai_success"] = "擦拭成功！",
  ["wzzz_v__pingcai_fail"] = "擦拭失败！",
  ["wzzz_v__pingcai_wolong"] = "卧龙",
  ["wzzz_v__pingcai_pangtong"] = "凤雏",
  ["wzzz_v__pingcai_simahui"] = "水镜",
  ["wzzz_v__pingcai_xushu"] = "玄剑",
  ["#wzzz_v__pingcai_wolong"] = "卧龙：对至多%arg名角色造成1点火焰伤害",
  ["#wzzz_v__pingcai_pangtong"] = "凤雏：横置至多%arg名角色的武将牌",
  ["#wzzz_v__pingcai_simahui"] = "水镜：移动场上的一张%arg",
  ["#wzzz_v__pingcai_xushu"] = "玄剑：令一名角色摸一张牌并回复1点体力",

  ["$wzzz_v__pingcai1"] = "吾有众好友，分为卧龙、凤雏、水镜、元直。",
  ["$wzzz_v__pingcai2"] = "孔明能借天火之势。",
  ["$wzzz_v__pingcai3"] = "士元虑事环环相扣。",
  ["$wzzz_v__pingcai4"] = "德操深谙处世之道。",
  ["$wzzz_v__pingcai5"] = "元直侠客惩恶扬善。",
}
...
pingcai:addEffect("active", {
  ...
  interaction = UI.ComboBox {choices = {"wzzz_v__pingcai_wolong", "wzzz_v__pingcai_pangtong", "wzzz_v__pingcai_simahui", "wzzz_v__pingcai_xushu"}},
  ...
  on_use = function(self, room, effect)
    ...
    if math.random() < 0.03 then  --看看哪个倒霉蛋失败
      room:doBroadcastNotify("ShowToast", Fk:translate("wzzz_v__pingcai_fail"))
      return
    end
    room:doBroadcastNotify("ShowToast", Fk:translate("wzzz_v__pingcai_success"))
    if self.interaction.data == "wzzz_v__pingcai_wolong" then
      ...
      local tos = room:askToChoosePlayers(player, {
        ...
        prompt = "#wzzz_v__pingcai_wolong:::"..n,
        cancelable = false,
      })
    ...
    elseif self.interaction.data == "wzzz_v__pingcai_pangtong" then
      local n = table.find(room.alive_players, function(p)
      ...
      local tos = room:askToChoosePlayers(player, {
        ...
        targets = targets,
        skill_name = pingcai.name,
        prompt = "#wzzz_v__pingcai_pangtong:::"..n,
        cancelable = false,
      })
      room:sortByAction(tos)
    ...
    elseif self.interaction.data == "wzzz_v__pingcai_simahui" then
      local pattern = "armor"
      ...
      local targets = room:askToChooseToMoveCardInBoard(player, {
        prompt = "#wzzz_v__pingcai_simahui:::"..pattern,
        skill_name = pingcai.name,
        no_indicate = true,
        flag = "e",
        cancelable = false,
    ...
    elseif self.interaction.data == "wzzz_v__pingcai_xushu" then
      local to = room:askToChoosePlayers(player, {
        min_num = 1,
        max_num = 1,
        targets = room.alive_players,
        skill_name = pingcai.name,
        prompt = "#wzzz_v__pingcai_xushu",
        cancelable = false,
      })
