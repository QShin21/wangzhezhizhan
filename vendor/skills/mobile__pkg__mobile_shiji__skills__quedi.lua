...
Fk:loadTranslationTable{
  ...
  ["wzzz_v__quedi_damage"] = "弃一张基本牌，令此【%arg】伤害+1",
  ["wzzz_v__quedi_beishui"] = "背水：减1点体力上限",
  ["#wzzz_v__quedi-discard"] = "却敌：你可以弃置一张基本牌，令此【%arg】伤害+1",

  ["$wzzz_v__quedi1"] = "力摧敌阵，如视天光破云！",
  ["$wzzz_v__quedi2"] = "让尔等有命追，无命回！",
}
...
quedi:addEffect(fk.TargetSpecified, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    if not choice:startsWith("wzzz_v__quedi_damage") and data.to:isAlive() and not data.to:isKongcheng() then
    ...
    if not choice:startsWith("wzzz_v__quedi_prey") then
    ...
    if choice == "wzzz_v__quedi_beishui" then
