...
Fk:loadTranslationTable{
  ["wzzz_v__daizui"] = "戴罪",
  [":wzzz_v__daizui"] = "限定技，当你受到致命伤害时，你可以防止此伤害，然后将对你造成伤害的牌置于伤害来源的武将牌上，称为“释”。" ..
  "本回合结束时，其获得其“释”。",

  ["wzzz_v__daizui_shi"] = "释",
...
daizui:addEffect(fk.DetermineDamageInflicted, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    if data.card and data.from and data.from:isAlive() and room:getCardArea(data.card) == Card.Processing then
      data.from:addToPile("wzzz_v__daizui_shi", data.card, true, daizui.name, player)
    end
  end,
})
...
daizui:addEffect(fk.TurnEnd, {
  ...
  mute = true,
  can_trigger = function(self, event, target, player, data)
    return #player:getPile("wzzz_v__daizui_shi") > 0
  end,
  on_cost = Util.TrueFunc,
  on_use = function(self, event, target, player, data)
    player.room:obtainCard(player, player:getPile("wzzz_v__daizui_shi"), true, fk.ReasonPrey, player, daizui.name)
  end,
})
