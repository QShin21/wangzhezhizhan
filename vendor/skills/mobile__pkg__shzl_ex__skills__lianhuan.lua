...
Fk:loadTranslationTable{
  ...
  ["$wzzz_v__m_ex__lianhuan2"] = "善用兵者，运巧必防损，立谋虑中变。",
}

lianhuan:addEffect("active", {
  mute = true,
  prompt = "#wzzz_v__m_ex__lianhuan",
  card_num = 1,
  min_target_num = 0,
...
lianhuan:addEffect(fk.AfterCardTargetDeclared, {
  ...
  on_cost = function (self, event, target, player, data)
    ...
    local to = room:askToChoosePlayers(player, {
      ...
      prompt = "#wzzz_v__m_ex__lianhuan-choose:::"..data.card:toLogString(),
      cancelable = true,
    })
    if #to > 0 then
