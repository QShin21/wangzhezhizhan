...
Fk:loadTranslationTable{
  ...
  [":wzzz_v__m_ex__qingnang"] = "出牌阶段限一次，你可以弃置一张手牌并选择一名已受伤角色，令其回复1点体力。若弃置的牌为红色，你可以再对本阶段"..
  "未选择过的角色发动〖青囊〗。",

  ["#wzzz_v__m_ex__qingnang"] = "青囊：弃置一张手牌，令一名角色回复1点体力，若弃置了红色牌则可以继续发动",
...
qingnang:addEffect("active", {
  ...
  target_filter = function(self, player, to_select, selected, cards)
    return #selected == 0 and to_select:isWounded() and not table.contains(player:getTableMark("wzzz_v__m_ex__qingnang-phase"), to_select.id)
  end,
  on_use = function(self, room, effect)
    ...
    if yes and not player.dead and not player:isKongcheng() then
      local targets = table.filter(room.alive_players, function (p)
        return p:isWounded() and not table.contains(player:getTableMark("wzzz_v__m_ex__qingnang-phase"), p.id)
      end)
      if #targets > 0 then
