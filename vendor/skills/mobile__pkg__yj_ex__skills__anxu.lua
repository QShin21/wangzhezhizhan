...
Fk:loadTranslationTable{
  ...
  [":wzzz_v__m_ex__anxu"] = "出牌阶段限一次，你可以令一名其他角色获得另一名其他角色的一张牌。若其获得的不是来自装备区里的牌，你摸一张牌。"..
    "当其以此法获得牌后，你可以令两者手牌较少的角色摸一张牌。",

  ["#wzzz_v__m_ex__anxu-active"] = "安恤：选择两名其他角色，令先选择的角色获得后选择的角色的一张牌",
  ["#wzzz_v__m_ex__anxu-draw"] = "安恤：是否令%dest摸一张牌",
  ...
  on_use = function(self, room, use)
    ...
    if not player.dead and not target1.dead and not target2.dead and target1:getHandcardNum() ~= target2:getHandcardNum() then
      ...
      if room:askToSkillInvoke(player, {
        skill_name = anxu.name,
        prompt = "#wzzz_v__m_ex__anxu-draw::" .. target1.id,
      }) then
