...
Fk:loadTranslationTable{
  ...
  ["@wzzz_v__m_ex__benxi-phase"] = "奔袭减距离",
  ["@@wzzz_v__m_ex__benxi-phase"] = "奔袭加目标",

  ["$wzzz_v__m_ex__benxi1"] = "战事唯论成败，何惜此等无用之物？",
  ["$wzzz_v__m_ex__benxi2"] = "汝等惊弓之鸟，亦难逃吾奔战穷击！",
  ["$wzzz_v__m_ex__benxi3"] = "袍染雍凉落日，马过岐山残雪！",
}
...
benxi:addEffect(fk.EventPhaseStart, {
  ...
  on_cost = function(self, event, target, player, data)
    local cards = player.room:askToDiscard(player, {
      ...
      cancelable = true,
      pattern = ".",
      prompt = "#wzzz_v__m_ex__benxi-discard",
      skip = true,
    })
    if #cards > 0 then
  ...
  on_use = function(self, event, target, player, data)
    ...
    room:addPlayerMark(player, "@wzzz_v__m_ex__benxi-phase", #cards)
    room:addPlayerMark(player, "@@wzzz_v__m_ex__benxi-phase")
  end,
})
...
benxi:addEffect(fk.AfterCardUseDeclared, {
  can_refresh = function(self, event, target, player, data)
    return target == player and (data.card.type == Card.TypeBasic or data.card:isCommonTrick()) and
      player:getMark("@@wzzz_v__m_ex__benxi-phase") > 0
  end,
  on_refresh = function(self, event, target, player, data)
    player.room:setPlayerMark(player, "@@wzzz_v__m_ex__benxi-phase", 0)
...
benxi:addEffect(fk.AfterCardTargetDeclared, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    local n = player:getMark("@wzzz_v__m_ex__benxi-phase")
    ...
    tos = room:askToChoosePlayers(player, {
      ...
      min_num = 1,
      max_num = n,
      prompt = "#wzzz_v__m_ex__benxi-choose:::"..data.card:toLogString()..":"..tostring(n),
      skill_name = benxi.name,
      cancelable = true
    })
    if #tos > 0 then
...
benxi:addEffect("distance", {
  correct_func = function(self, from, to)
    return -from:getMark("@wzzz_v__m_ex__benxi-phase")
  end,
})

benxi:addLoseEffect(function(self, player)
  local room = player.room
  room:setPlayerMark(player, "@wzzz_v__m_ex__benxi-phase", 0)
  room:setPlayerMark(player, "@@wzzz_v__m_ex__benxi-phase", 0)
end)
