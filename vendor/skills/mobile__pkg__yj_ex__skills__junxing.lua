local junxing = fk.CreateSkill {
  name = "wzzz_v__m_ex__junxing",
}

Fk:loadTranslationTable{
  ["wzzz_v__m_ex__junxing"] = "峻刑",
  [":wzzz_v__m_ex__junxing"] = "出牌阶段限一次，你可以弃置任意张手牌并令一名其他角色选择一项：1.弃置等量的牌并失去1点体力；2.翻面，然后摸等量的牌。",

  ["#wzzz_v__m_ex__junxing-active"] = "发动峻刑，选择任意张手牌弃置并选择一名其他角色",
  ["#wzzz_v__m_ex__junxing-discard"] = "峻刑：选择弃置%arg张牌并失去1点体力，或点取消则翻面并摸%arg张牌",

  ["$wzzz_v__m_ex__junxing1"] = "严法尚公，岂分贵贱而异施？",
  ["$wzzz_v__m_ex__junxing2"] = "情理可容之事，法未必能容！",
}

junxing:addEffect("active", {
  anim_type = "control",
  prompt = "#wzzz_v__m_ex__junxing-active",
  ...
  on_use = function(self, room, effect)
    ...
    if #room:askToDiscard(target, {
      ...
      cancelable = true,
      pattern = ".",
      prompt = "#wzzz_v__m_ex__junxing-discard:::"..x
    }) == 0 then
      target:turnOver()
      if target.dead then return end
