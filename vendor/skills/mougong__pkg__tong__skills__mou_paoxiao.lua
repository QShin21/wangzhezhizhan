local mouPaoxiao = fk.CreateSkill({
  name = "wzzz_v__mou__paoxiao",
  tags = { Skill.Compulsory },
})

Fk:loadTranslationTable{
  ["wzzz_v__mou__paoxiao"] = "咆哮",
  [":wzzz_v__mou__paoxiao"] = "锁定技，①你使用【杀】无次数限制；"..
  "②若你装备了武器牌，你使用【杀】无距离限制；"..
  "③当你于出牌阶段使用【杀】指定目标后，若你本阶段已使用过【杀】，你令目标角色本回合非锁定技失效，" ..
  "此【杀】不能被响应且【杀】伤害值+1，此【杀】对目标角色造成伤害后若其未死亡，你失去1点体力并随机弃置一张手牌。",
  ["@@wzzz_v__mou__paoxiao-turn"] = "咆哮封技",
  ["#wzzz_v__mou__paoxiao_delay"] = "咆哮",

  ["$wzzz_v__mou__paoxiao1"] = "我乃燕人张飞，尔等休走！",
  ["$wzzz_v__mou__paoxiao2"] = "战又不战，退又不退，却是何故！",
}
...
mouPaoxiao:addEffect(fk.TargetSpecified, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    local to = data.to
    room:addPlayerMark(to, "@@wzzz_v__mou__paoxiao-turn")
...
mouPaoxiao:addEffect(fk.Damage, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    if #cards > 0 then
      room:throwCard(room:tableRandomPick(cards, 1), "wzzz_v__mou__paoxiao", player, player)
    end
  end,
})
