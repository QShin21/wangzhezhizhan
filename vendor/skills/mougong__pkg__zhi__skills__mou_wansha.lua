local mouWansha = fk.CreateSkill({
  name = "wzzz_v__mou__wansha",
})

Fk:loadTranslationTable{
  ["wzzz_v__mou__wansha"] = "完杀",
  [":wzzz_v__mou__wansha"] = "①你的回合内，若有角色处于濒死状态，则不处于濒死状态的其他角色不能使用【桃】。"..
  "<br>②每轮限一次，一名角色进入濒死状态时，你可以观看其手牌并秘密选择其中的0~2张牌，然后令其选择一项：1.由你将被选择的牌分配给除其以外的角色；2.弃置所有未被选择的牌。"..
  "<br><b>二级</b>：“选择其手牌”修改为“选择其区域内牌”。",
  ["@@wzzz_v__mou__wansha_upgrade"] = "完杀二级",
  ["#wzzz_v__mou__wansha-invoke"] = "完杀：你可以观看 %src 手牌并选牌，令其选择让你分配之或弃置其余牌",
  ["#wzzz_v__mou__wansha_give"] = "令其将选择的牌分配",
  ["#wzzz_v__mou__wansha_throw"] = "弃置其未选择的牌",
  ["#wzzz_v__mou__wansha-choice"] = "完杀：%src 秘密选择了你的若干张牌，你须选一项",

  ["$wzzz_v__mou__wansha1"] = "世人皆行殊途，与死亦有同归！",
  ["$wzzz_v__mou__wansha2"] = "九幽泉下，是你最好的归宿。",
}
...
mouWansha:addEffect(fk.EnterDying, {
  ...
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill(mouWansha.name) and player:usedSkillTimes(mouWansha.name, Player.HistoryRound) == 0 then
      if player:getMark("@@wzzz_v__mou__wansha_upgrade") == 0 then
  ...
  on_cost = function (self, event, target, player, data)
    return player.room:askToSkillInvoke(player, { skill_name = mouWansha.name, prompt = "#wzzz_v__mou__wansha-invoke:" .. target.id })
  end,
  on_use = function(self, event, target, player, data)
    ...
    local card_data = {}
    local upgrade = player:getMark("@@wzzz_v__mou__wansha_upgrade") > 0
    if upgrade then
    ...
    local choice = room:askToChoice(
      target,
      {
        choices = { "#wzzz_v__mou__wansha_give", "#wzzz_v__mou__wansha_throw" },
        skill_name = skillName,
        prompt = "#wzzz_v__mou__wansha-choice:" .. player.id
      }
    )
    if choice == "#wzzz_v__mou__wansha_give" then
...
mouWansha:addLoseEffect(function (self, player)
  if player:getMark("@@wzzz_v__mou__wansha_upgrade") > 0 then
    player.room:setPlayerMark(player, "@@wzzz_v__mou__wansha_upgrade", 0)
  end
end)
