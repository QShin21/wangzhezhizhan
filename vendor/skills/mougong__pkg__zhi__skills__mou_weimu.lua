local mouWeimu = fk.CreateSkill({
  name = "wzzz_v__mou__weimu",
  tags = { Skill.Compulsory },
})

Fk:loadTranslationTable{
  ["wzzz_v__mou__weimu"] = "帷幕",
  [":wzzz_v__mou__weimu"] = "锁定技，当你成为黑色锦囊牌的目标时，取消之。"..
  "<br><b>二级</b>：增加内容：每轮开始时，若你上一轮成为其他角色使用牌的目标的次数不大于1次，则你从弃牌堆随机获得一张黑色锦囊牌或者防具牌。",
  ["@@wzzz_v__mou__weimu_upgrade"] = "帷幕二级",

  ["$wzzz_v__mou__weimu1"] = "执棋之人，不可与入局者共论。",
  ["$wzzz_v__mou__weimu2"] = "世有千万门法，与我均无纠葛。",
  ["$wzzz_v__mou__weimu3"] = "方圆之间，参透天地万物心！",
  ["$wzzz_v__mou__weimu4"] = "帐前独知行表，幕后可见人心！",
}
...
mouWeimu:addEffect(fk.RoundStart, {
  anim_type = "drawcard",
  can_trigger = function(self, event, target, player, data)
    if player:getMark("@@wzzz_v__mou__weimu_upgrade") > 0 then
...
mouWeimu:addLoseEffect(function (self, player)
  if player:getMark("@@wzzz_v__mou__weimu_upgrade") > 0 then
    player.room:setPlayerMark(player, "@@wzzz_v__mou__weimu_upgrade", 0)
  end
end)
