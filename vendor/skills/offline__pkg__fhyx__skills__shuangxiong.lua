local shuangxiong = fk.CreateSkill {
  name = "wzzz_v__fhyx_ex__shuangxiong",
}
...
Fk:loadTranslationTable{
  ...
  ["#wzzz_v__fhyx_ex__shuangxiong-choose"] = "双雄：令一名角色弃置一张牌，此阶段你可以将与之颜色不同的手牌当【决斗】使用",
  ["#wzzz_v__fhyx_ex__shuangxiong-discard"] = "双雄：请弃置一张牌，此阶段 %src 可以将与之颜色不同的手牌当【决斗】使用",
}
...
shuangxiong:addEffect("viewas", {
  ...
  prompt = function(self, player)
    local color = "red"
    if player:getMark("@wzzz_v__fhyx_ex__shuangxiong-phase") == "red" then
      color = "black"
    end
    return "#wzzz_v__fhyx_ex__shuangxiong:::"..color
  end,
  handly_pile = true,
  card_filter = function(self, player, to_select, selected)
    ...
    return table.contains(player:getHandlyIds(), to_select) and player:getMark("@wzzz_v__fhyx_ex__shuangxiong-phase") == color
  end,
  ...
  enabled_at_play = function(self, player)
    return player:getMark("@wzzz_v__fhyx_ex__shuangxiong-phase") ~= 0
  end,
  enabled_at_response = function(self, player, response)
    return not response and player:getMark("@wzzz_v__fhyx_ex__shuangxiong-phase") ~= 0
  end,
})
...
shuangxiong:addEffect(fk.EventPhaseStart, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local to = room:askToChoosePlayers(player, {
      ...
      min_num = 1,
      max_num = 1,
      prompt = "#wzzz_v__fhyx_ex__shuangxiong-choose",
  ...
  on_use = function(self, event, target, player, data)
    ...
    local card = room:askToDiscard(to, {
      ...
      include_equip = true,
      skill_name = shuangxiong.name,
      prompt = "#wzzz_v__fhyx_ex__shuangxiong-discard:"..player.id,
      cancelable = false,
    })
    if #card > 0 and not player.dead then
      local color = Fk:getCardById(card[1]):getColorString()
      if color ~= "nocolor" then
        room:setPlayerMark(player, "@wzzz_v__fhyx_ex__shuangxiong-phase", color)
      end
    end
  end,
})
