...
local dimeng = fk.CreateSkill{
  name = "wzzz_v__ol_ex__dimeng",
}

Fk:loadTranslationTable {
  ["wzzz_v__ol_ex__dimeng"] = "缔盟",
  [":wzzz_v__ol_ex__dimeng"] = "出牌阶段限一次，你可以选择两名手牌数之差不大于你的牌数的其他角色，这两名角色交换手牌。此阶段结束时，你弃置X张牌"..
  "（X为这两名角色手牌数之差）。",

  ["#wzzz_v__ol_ex__dimeng"] = "缔盟：令两名其他角色交换手牌（手牌数之差不大于你的牌数）",

  ["$wzzz_v__ol_ex__dimeng1"] = "深知其奇，相与亲结。",
  ["$wzzz_v__ol_ex__dimeng2"] = "同盟之人，言归于好。",
}

dimeng:addEffect("active", {
  anim_type = "control",
  prompt = "#wzzz_v__ol_ex__dimeng",
  card_num = 0,
  target_num = 2,
  ...
  on_use = function(self, room, effect)
    ...
    room:addTableMark(player, "wzzz_v__ol_ex__dimeng_target-phase", table.map(effect.tos, Util.IdMapper))
...
dimeng:addEffect(fk.EventPhaseEnd, {
  ...
  can_trigger = function(self, event, target, player, data)
    if not player.dead and player:usedSkillTimes(dimeng.name, Player.HistoryPhase) > 0 and not player:isNude() then
      for _, info in ipairs(player:getTableMark("wzzz_v__ol_ex__dimeng_target-phase")) do
