local huangtian_active = fk.CreateSkill {
  name = "wzzz_v__ol_ex__huangtian_active&",
}
...
huangtian_active:addEffect("active", {
  anim_type = "support",
  prompt = "#wzzz_v__ol_ex__huangtian",
  mute = true,
  card_num = 1,
  target_num = 0,
  can_use = function(self, player)
    if player:usedSkillTimes(huangtian_active.name, Player.HistoryPhase) < 1 and player.kingdom == "qun" then
      return table.find(Fk:currentRoom().alive_players, function(p) return p:hasSkill("wzzz_v__ol_ex__huangtian") and p ~= player end)
    end
    return false
  end,
  ...
  on_use = function(self, room, effect)
    ...
    local targets = table.filter(room.alive_players, function(p)
      return p:hasSkill("wzzz_v__ol_ex__huangtian") and p ~= player
    end)
    local target
    if #targets == 1 then
      target = targets[1]
      ...
      target = room:askToChoosePlayers(player, {
        ...
        max_num = 1,
        targets = targets,
        skill_name = "wzzz_v__ol_ex__huangtian",
        prompt = "#wzzz_v__ol_ex__huangtian",
        cancelable = false,
      })[1]
    end
    if not target then return end
    room:notifySkillInvoked(target, "wzzz_v__ol_ex__huangtian")
    target:broadcastSkillInvoke("wzzz_v__ol_ex__huangtian")
    room:doIndicate(player.id, { target.id })
    room:moveCardTo(effect.cards, Player.Hand, target, fk.ReasonGive, "wzzz_v__ol_ex__huangtian", nil, true, player)
  end,
})
