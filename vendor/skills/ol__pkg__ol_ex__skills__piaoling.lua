local piaoling = fk.CreateSkill {
  name = "wzzz_v__ol_ex__piaoling",
}

Fk:loadTranslationTable {
  ["wzzz_v__ol_ex__piaoling"] = "飘零",
  [":wzzz_v__ol_ex__piaoling"] = "结束阶段，你可判定，然后当判定结果确定后，若为<font color='red'>♥</font>，你选择：1.将判定牌置于牌堆顶；"..
  "2.令一名角色获得判定牌，若其为你，你弃置一张牌。",

  ["#wzzz_v__ol_ex__piaoling-choose"] = "飘零：令一名角色获得判定牌（若为你，你弃置一张牌），或点“取消”将判定牌置于牌堆顶",

  ["$wzzz_v__ol_ex__piaoling1"] = "花自飘零水自流。",
  ["$wzzz_v__ol_ex__piaoling2"] = "清风拂枝，落花飘零。",
}
  ...
  on_use = function(self, event, target, player, data)
    ...
    if data.card.suit == Card.Heart and room:getCardArea(data.card) == Card.Processing then
      local targets = room:askToChoosePlayers(player, {
        ...
        min_num = 1,
        max_num = 1,
        prompt = "#wzzz_v__ol_ex__piaoling-choose",
