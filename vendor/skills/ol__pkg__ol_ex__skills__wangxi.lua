local wangxi = fk.CreateSkill {
  name = "wzzz_v__ol_ex__wangxi",
}

Fk:loadTranslationTable{
  ["wzzz_v__ol_ex__wangxi"] = "忘隙",
  [":wzzz_v__ol_ex__wangxi"] = "当你对其他角色造成1点伤害后，或当你受到其他角色造成的1点伤害后，你可以摸两张牌，然后将其中一张牌交给该角色。",

  ["#wzzz_v__ol_ex__wangxi-invoke"] = "忘隙：是否对 %dest 发动“忘隙”，摸两张牌并将其中一张牌交给其",
  ["#wzzz_v__ol_ex__wangxi-give"] = "忘隙：将其中一张牌交给 %dest",

  ["$wzzz_v__ol_ex__wangxi1"] = "以德报怨，怨消恨解。",
  ["$wzzz_v__ol_ex__wangxi2"] = "冤冤相报，何时能了。",
}
...
wangxi:addEffect(fk.Damage, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      ...
      prompt = "#wzzz_v__ol_ex__wangxi-invoke::"..data.to.id,
    }) then
  ...
  on_use = function(self, event, target, player, data)
    ...
    local card = room:askToCards(player, {
      ...
      prompt = "#wzzz_v__ol_ex__wangxi-give::"..data.to.id,
      cancelable = false,
    })
    if #card > 0 then
...
wangxi:addEffect(fk.Damaged, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      skill_name = wangxi.name,
      prompt = "#wzzz_v__ol_ex__wangxi-invoke::"..data.from.id,
    }) then
  ...
  on_use = function(self, event, target, player, data)
    ...
    local card = room:askToCards(player, {
      ...
      skill_name = wangxi.name,
      pattern = tostring(Exppattern{ id = cards }),
      prompt = "#wzzz_v__ol_ex__wangxi-give::"..data.from.id,
      cancelable = false,
    })
    if #card > 0 then
