local enyuan = fk.CreateSkill{
  name = "wzzz_v__ol_ex__enyuan",
}

Fk:loadTranslationTable{
  ["wzzz_v__ol_ex__enyuan"] = "恩怨",
  [":wzzz_v__ol_ex__enyuan"] = "当你获得一名其他角色至少两张牌后，你可以令其摸一张牌。当你受到1点伤害后，你可以令伤害来源选择一项：1.交给你一张红色手牌；"..
  "2.失去1点体力。",

  ["#wzzz_v__ol_ex__enyuan1-invoke"] = "恩怨：是否令 %dest 摸一张牌？",
  ["#wzzz_v__ol_ex__enyuan2-invoke"] = "恩怨：是否令 %dest 选择交给你牌或失去体力？",
  ["#wzzz_v__ol_ex__enyuan-give"] = "恩怨：交给 %src 一张红色手牌，否则你失去1点体力",

  ["$wzzz_v__ol_ex__enyuan1"] = "恩重如山，必报之以雷霆之势！",
  ["$wzzz_v__ol_ex__enyuan2"] = "怨深似海，必还之以烈火之怒！",
}
...
enyuan:addEffect(fk.AfterCardsMove, {
  ...
  on_cost = function(self, event, target, player, data)
    if player.room:askToSkillInvoke(player, {
      ...
      prompt = "#wzzz_v__ol_ex__enyuan1-invoke::" .. target.id,
    }) then
      return true
    end
  end,
...
enyuan:addEffect(fk.Damaged, {
  ...
  on_cost = function (self, event, target, player, data)
    if player.room:askToSkillInvoke(player, {
      skill_name = enyuan.name,
      prompt = "#wzzz_v__ol_ex__enyuan2-invoke::"..data.from.id,
    }) then
