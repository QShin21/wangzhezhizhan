local qieting = fk.CreateSkill{
  name = "wzzz_v__ol_ex__qieting",
}

Fk:loadTranslationTable{
  ["wzzz_v__ol_ex__qieting"] = "窃听",
  [":wzzz_v__ol_ex__qieting"] = "其他角色的回合结束后，若其于此回合内未对其以外的角色造成过伤害或未对其以外的角色使用过牌，你可以选择："..
  "1.将其装备区的一张牌置入你的装备区；2.摸一张牌。若其于此回合内未对其以外的角色使用过牌，你可以执行另一项。",

  ["#wzzz_v__ol_ex__qieting-invoke"] = "窃听：可以移动装备或摸牌",
  ["wzzz_v__ol_ex__qieting_move"] = "将%dest一张装备移动给你",

  ["$wzzz_v__ol_ex__qieting1"] = "好你个刘玄德，敢坏我儿大事。",
  ["$wzzz_v__ol_ex__qieting2"] = "两个大男人窃窃私语，定没有好事。",
}
...
qieting:addEffect(fk.TurnEnd, {
  ...
  on_cost = function (self, event, target, player, data)
    local all_choices = {"wzzz_v__ol_ex__qieting_move::"..target.id, "draw1", "Cancel"}
    ...
    local choice = player.room:askToChoice(player, {
      ...
      prompt = "#wzzz_v__ol_ex__qieting-invoke",
    ...
    if event:getCostData(self).choice == "draw1" then
      ...
      if #room.logic:getEventsOfScope(GameEvent.UseCard, 1, function (e)
        ...
        if room:askToChoice(player, {
          choices = {"wzzz_v__ol_ex__qieting_move::"..target.id, "Cancel"},
          skill_name = qieting.name,
          prompt = "#wzzz_v__ol_ex__qieting-invoke",
          all_choices = all_choices,
        }) ~= "Cancel" then
