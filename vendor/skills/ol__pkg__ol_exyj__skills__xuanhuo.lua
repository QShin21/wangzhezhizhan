...
Fk:loadTranslationTable{
  ...
  ["$wzzz_v__ol_ex__xuanhuo2"] = "以眩惑试人心，以真情待贤才，方能得天下。",
}
...
xuanhuo:addEffect(fk.EventPhaseEnd, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local success, dat = room:askToUseActiveSkill(player, {
      skill_name = "wzzz_v__ol_ex__xuanhuo_active",
      prompt = "#wzzz_v__ol_ex__xuanhuo-invoke",
      cancelable = true,
    })
  ...
  on_use = function(self, event, target, player, data)
    ...
    local use = room:askToUseCard(to, {
      pattern = "slash",
      prompt = "#wzzz_v__ol_ex__xuanhuo-use:"..player.id..":"..victim.id,
      extra_data = {
    ...
    if use then
      ...
      local cards = room:askToChooseCards(player, {
        ...
        prompt = "#wzzz_v__ol_ex__xuanhuo-prey::"..to.id,
        skill_name = xuanhuo.name,
      })
