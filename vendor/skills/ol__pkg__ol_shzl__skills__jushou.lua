...
Fk:loadTranslationTable{
  ...
  ["$wzzz_v__ol__jushou1"] = "兵精粮足，守土一方。",
  ["$wzzz_v__ol__jushou2"] = "坚守此地，不退半步。",
}
    ...
    if card.type == Card.TypeEquip and player:canUse(card) then
      if player:canUseTo(card, player) then
        ...
        room:askToUseRealCard(player, {
          pattern = {card.id},
          skill_name = jushou.name,
          prompt = "#wzzz_v__ol__jushou-use:::"..card:toLogString(),
          cancelable = false,
        })
      end
    else
