local lijun = fk.CreateSkill {
  name = "wzzz_v__ol__lijun",
  tags = { Skill.Lord },
}

Fk:loadTranslationTable{
  ["wzzz_v__ol__lijun"] = "立军",
  [":wzzz_v__ol__lijun"] = "主公技，其他吴势力角色的出牌阶段内限一次，当其使用【杀】结算结束后，其可以将此【杀】交给你，然后你可以令其摸一张牌且"..
  "其此阶段使用【杀】次数上限+1。",

  ["#wzzz_v__ol__lijun-invoke"] = "立军：你可以将此【杀】交给 %src，然后其可令你摸一张牌",
  ["#wzzz_v__ol__lijun-draw"] = "立军：你可以令 %src 摸一张牌且使用【杀】次数+1",

  ["$wzzz_v__ol__lijun1"] = "能征善战，乃我东吴长久之风。",
  ["$wzzz_v__ol__lijun2"] = "重赏之下，必有勇夫。",
}
...
lijun:addEffect(fk.CardUseFinished, {
  ...
  on_cost = function (self, event, target, player, data)
    return player.room:askToSkillInvoke(target, {
      ...
      prompt = "#wzzz_v__ol__lijun-invoke:"..player.id,
    })
  end,
  on_use = function(self, event, target, player, data)
    ...
    if not player.dead and not target.dead and
      ...
      skill_name = lijun.name,
      prompt = "#wzzz_v__ol__lijun-draw:"..target.id,
    }) then
