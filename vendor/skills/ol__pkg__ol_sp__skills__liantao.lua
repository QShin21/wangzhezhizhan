local liantao = fk.CreateSkill{
  name = "wzzz_v__liantao",
}

Fk:loadTranslationTable{
  ["wzzz_v__liantao"] = "连讨",
  [":wzzz_v__liantao"] = "出牌阶段开始时，你可令一名其他角色选择一种颜色，然后你展示所有手牌，"..
    "并依次将此颜色的手牌当【决斗】对其使用直到有一方进入濒死状态，然后你摸X张牌（X为你以此法对目标角色造成的伤害数）。"..
    "若该角色没有因此受到伤害，你摸一张牌，本回合手牌上限+1且不能使用【杀】。",

  ["#wzzz_v__liantao-choose"] = "连讨：选择一名其他角色，将一种颜色的手牌当【决斗】对其使用！",
  ["#wzzz_v__liantao-choice"] = "连讨：选择一种颜色，%src 将此颜色手牌当【决斗】对你使用！",
  ["#wzzz_v__liantao-duel"] = "连讨：选择一张%arg手牌当【决斗】对 %dest 使用",

  ["$wzzz_v__liantao1"] = "沙场百战疾，争衡天下间。",
  ["$wzzz_v__liantao2"] = "征战无休，决胜千里。",
}
...
liantao:addEffect(fk.EventPhaseStart, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    local choice = room:askToChoice(to, {
      choices = {"red", "black"},
      skill_name = liantao.name,
      prompt = "#wzzz_v__liantao-choice:"..player.id,
    })
    ...
    while not (player.dead or player:isKongcheng() or to.dead) do
      ...
      local card = room:askToCards(player, {
        ...
        prompt = "#wzzz_v__liantao-duel::"..to.id..":"..choice,
        cancelable = false,
      })
    ...
    if x > 0 then
      ...
      room:setPlayerMark(player, "wzzz_v__liantao_prohibit-turn", 1)
...
liantao:addEffect("prohibit", {
  prohibit_use = function(self, player, card)
    return player:getMark("wzzz_v__liantao_prohibit-turn") > 0 and card.trueName == "slash"
  end,
})
