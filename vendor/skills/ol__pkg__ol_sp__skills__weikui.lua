...
Fk:loadTranslationTable{
  ["wzzz_v__weikui"] = "伪溃",
  [":wzzz_v__weikui"] = "出牌阶段限一次，你可以失去1点体力并选择一名有手牌的其他角色观看其手牌：若其手牌中有【闪】，则视为你对其使用一张"..
  "不计入次数限制的【杀】，且本回合你计算与其的距离视为1；若其手牌中没有【闪】，你弃置其中一张牌。",

  ["#wzzz_v__weikui"] = "伪溃：失去1点体力观看一名角色手牌，根据有无【闪】视为对其使用【杀】或弃置其牌",
  ["#wzzz_v__weikui-discard"] = "伪溃：弃置 %dest 一张手牌",

  ["$wzzz_v__weikui1"] = "骑兵列队，准备突围。",
  ["$wzzz_v__weikui2"] = "休整片刻，且随我杀出一条血路。",
}

weikui:addEffect("active", {
  anim_type = "offensive",
  prompt = "#wzzz_v__weikui",
  card_num = 0,
  ...
  on_use = function(self, room, effect)
    ...
    if table.find(target:getCardIds("h"), function(id)
      ...
      room:addTableMark(player, "wzzz_v__weikui-turn", target.id)
      ...
      local card = room:askToChooseCard(player, {
        ...
        prompt = "#wzzz_v__weikui-discard::"..target.id,
      })
...
weikui:addEffect("distance", {
  fixed_func = function(self, from, to)
    if table.contains(from:getTableMark("wzzz_v__weikui-turn"), to.id) then
      return 1
    end
  end,
})
