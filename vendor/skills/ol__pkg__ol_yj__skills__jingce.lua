local jingce = fk.CreateSkill {
  name = "wzzz_v__ol__jingce",
}

Fk:loadTranslationTable {
  ["wzzz_v__ol__jingce"] = "精策",
  [":wzzz_v__ol__jingce"] = "你每于回合内使用一种花色的手牌，本回合的手牌上限便+1；出牌阶段结束时，你可以摸X张牌（X为你本回合使用过牌的类别数）。",
  ["@[suits]wzzz_v__ol__jingce-turn"] = "精策",
  ["$wzzz_v__ol__jingce1"] = "良策佐君王，率征万精兵。",
  ["$wzzz_v__ol__jingce2"] = "得一寸，进一尺。",
}
...
jingce:addEffect(fk.CardUsing, {
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(jingce.name) and
        ...
        not table.contains(player:getTableMark("wzzz_v__ol__jingce-turn"), data.card.suit)
  end,
  on_cost = Util.TrueFunc,
  on_use = function(self, event, target, player, data)
    local room = player.room
    room:addPlayerMark(player, MarkEnum.AddMaxCardsInTurn, 1)
    room:addTableMark(player, "@[suits]wzzz_v__ol__jingce-turn", data.card.suit)
  end,
})
