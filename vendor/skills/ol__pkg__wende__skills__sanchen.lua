...
Fk:loadTranslationTable{
  ...
  ["$wzzz_v__sanchen1"] = "陈书弼国，当一而再、再而三。",
  ["$wzzz_v__sanchen2"] = "勘除弼事，三陈而就。",
}

sanchen:addEffect("active", {
  anim_type = "drawcard",
  prompt = "#wzzz_v__sanchen",
  card_num = 0,
  target_num = 1,
  ...
  target_filter = function(self, player, to_select, selected)
    return #selected == 0 and not table.contains(player:getTableMark("wzzz_v__sanchen-turn"), to_select.id)
  end,
  on_use = function(self, room, effect)
    local player = effect.from
    local target = effect.tos[1]
    if player:hasSkill("wzzz_v__zhaotao", true) and player:usedSkillTimes("wzzz_v__zhaotao", Player.HistoryGame) == 0 and
      player:getMark("@wzzz_v__sanchen") < 3 then
      room:addPlayerMark(player, "@wzzz_v__sanchen")
    end
    room:addTableMark(player, "wzzz_v__sanchen-turn", target.id)
    ...
    local cards = room:askToDiscard(target, {
      ...
      prompt = "#wzzz_v__sanchen-discard:"..player.id,
      cancelable = false,
      skip = true,
    })
    local typeMap = {}
