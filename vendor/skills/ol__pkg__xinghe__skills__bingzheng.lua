local bingzheng = fk.CreateSkill{
  name = "wzzz_v__bingzheng",
}

Fk:loadTranslationTable{
  ["wzzz_v__bingzheng"] = "秉正",
  [":wzzz_v__bingzheng"] = "出牌阶段结束时，你可以令手牌数不等于体力值的一名角色弃置一张手牌或摸一张牌。然后若其手牌数等于体力值，你摸一张牌，"..
  "且可以交给该角色一张牌。",

  ["#wzzz_v__bingzheng-choose"] = "秉正：令一名角色执行一项，然后若其手牌数等于体力值，你摸一张牌且可以交给其一张牌",
  ["#wzzz_v__bingzheng-give"] = "秉正：你可以交给 %dest 一张牌",

  ["$wzzz_v__bingzheng1"] = "自古，就是邪不胜正！",
  ["$wzzz_v__bingzheng2"] = "主公面前，岂容小人搬弄是非！",
}
...
bingzheng:addEffect(fk.EventPhaseEnd, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local success, dat = room:askToUseActiveSkill(player, {
      skill_name = "wzzz_v__bingzheng_active",
      prompt = "#wzzz_v__bingzheng-choose",
      cancelable = true,
    })
    ...
    if to:getHandcardNum() == to.hp then
      ...
      if to ~= player and not to.dead and not player.dead then
        local card = room:askToCards(player, {
          ...
          include_equip = true,
          skill_name = bingzheng.name,
          prompt = "#wzzz_v__bingzheng-give::"..to.id,
          cancelable = true,
        })
        if #card > 0 then
