...
Fk:loadTranslationTable{
  ...
  [":wzzz_v__ol__cuorui"] = "锁定技，游戏开始时，你将手牌数摸至X张（X为场上角色数）。当你成为延时锦囊牌的目标后，你跳过下个判定阶段。",
...
cuorui:addEffect(fk.TargetConfirmed, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(cuorui.name) and data.card.sub_type == Card.SubtypeDelayedTrick and
      player:getMark("@@wzzz_v__ol__cuorui") == 0
  end,
  on_use = function(self, event, target, player, data)
    player.room:setPlayerMark(player, "@@wzzz_v__ol__cuorui", 1)
  end,
})
cuorui:addEffect(fk.EventPhaseChanging, {
  can_refresh = function (self, event, target, player, data)
    return target == player and data.phase == Player.Judge and
      player:getMark("@@wzzz_v__ol__cuorui") > 0 and not data.skipped
  end,
  on_refresh = function (self, event, target, player, data)
    player.room:setPlayerMark(player, "@@wzzz_v__ol__cuorui", 0)
    data.skipped = true
  end,
})
