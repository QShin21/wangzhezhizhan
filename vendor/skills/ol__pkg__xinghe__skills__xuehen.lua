local xuehen = fk.CreateSkill{
  name = "wzzz_v__ol__xuehen",
}

Fk:loadTranslationTable{
  ["wzzz_v__ol__xuehen"] = "雪恨",
  [":wzzz_v__ol__xuehen"] = "出牌阶段限一次，你可以弃置一张红色牌并选择至多X名角色（X为你已损失的体力值且至少为1），你横置这些角色，"..
  "然后对其中一名角色造成1点火焰伤害。",

  ["#wzzz_v__ol__xuehen"] = "雪恨：弃置一张红色牌并选择至多%arg名角色，横置这些角色，然后对其中一名角色造成1点火焰伤害",
  ["#wzzz_v__ol__xuehen-choose"] = "雪恨：对其中一名角色造成1点火焰伤害",

  ["$wzzz_v__ol__xuehen1"] = "就用你的性命，一雪前耻。",
  ["$wzzz_v__ol__xuehen2"] = "雪耻旧恨，今日清算。",
}

xuehen:addEffect("active", {
  anim_type = "offensive",
  prompt = function (self, player)
    return "#wzzz_v__ol__xuehen:::"..math.max(1, player:getLostHp())
  end,
  card_num = 1,
  min_target_num = 1,
  ...
  on_use = function(self, room, effect)
    ...
    if #targets > 1 then
      to = room:askToChoosePlayers(player, {
        ...
        prompt = "#wzzz_v__ol__xuehen-choose",
        cancelable = false,
      })[1]
    end
    room:damage{
