local osZhiming = fk.CreateSkill {
  name = "wzzz_v__os__zhiming"
}

Fk:loadTranslationTable{
  ["wzzz_v__os__zhiming"] = "知命",
  [":wzzz_v__os__zhiming"] = "准备阶段开始时或弃牌阶段结束时，你摸一张牌，然后你可将一张牌置于牌堆顶。",

  ["#wzzz_v__os__zhiming-ask"] = "知命：你可将一张牌置于牌堆顶",

  ["$wzzz_v__os__zhiming1"] = "天定人命，仅可一窥。",
  ["$wzzz_v__os__zhiming2"] = "知命而行，尽诸人事。",
}
...
local osZhimingOnUse = function(self, event, target, player)
  ...
  local cids = room:askToCards(
    player,
    {
      ...
      prompt = "#wzzz_v__os__zhiming-ask",
    }
  )
  if #cids > 0 then
