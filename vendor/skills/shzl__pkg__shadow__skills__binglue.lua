local binglue = fk.CreateSkill {
  name = "wzzz_v__binglue",
  tags = {Skill.Compulsory},
}

Fk:loadTranslationTable{
  ["wzzz_v__binglue"] = "兵略",
  [":wzzz_v__binglue"] = "锁定技，当你首次对一名角色发动〖飞军〗结算后，你摸两张牌。",

  ["$wzzz_v__binglue1"] = "奇略兵速，敌未能料之。",
  ["$wzzz_v__binglue2"] = "兵略者，明战胜攻取之数，形机之势，诈谲之变。",
}
...
binglue:addEffect(fk.AfterCardsMove, {
  ...
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill(binglue.name) then
      ...
      if e and e.data.who == player and e.data.skill.name == "wzzz_v__feijun" then
