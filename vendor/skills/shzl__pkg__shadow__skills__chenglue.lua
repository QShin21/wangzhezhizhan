...
Fk:loadTranslationTable{
  ["wzzz_v__chenglue"] = "成略",
  [":wzzz_v__chenglue"] = "转换技，出牌阶段限一次，阳：你可以摸一张牌，然后弃置两张手牌；阴：你可以摸两张牌，然后弃置一张手牌。"..
  "若如此做，你于此阶段内使用与你以此法弃置的牌花色相同的牌无距离和次数限制。",

  [":wzzz_v__chenglue_yang"] = "转换技，出牌阶段限一次，<font color=\"#E0DB2F\">阳：你可以摸一张牌，然后弃置两张手牌；</font>" ..
  "阴：你可以摸两张牌，然后弃置一张手牌。若如此做，你于此阶段内使用与你以此法弃置的牌花色相同的牌无距离和次数限制。",
  [":wzzz_v__chenglue_yin"] = "转换技，出牌阶段限一次，阳：你可以摸一张牌，然后弃置两张手牌；"..
  "<font color=\"#E0DB2F\">阴：你可以摸两张牌，然后弃置一张手牌。</font>若如此做，你于此阶段内使用与你以此法弃置的牌花色相同的牌无距离和次数限制。",

  ["#wzzz_v__chenglue-active"] = "成略：摸%arg张牌，弃置%arg2张手牌，本阶段使用这些花色的牌无距离次数限制",
  ["#wzzz_v__chenglue-discard"] = "成略：弃置%arg张手牌，本阶段使用这些花色的牌无距离和次数限制",
  ["@wzzz_v__chenglue-phase"] = "成略",
...
wzzz_v__chenglue:addEffect("active", {
  ...
  prompt = function (self, player)
    return player:getSwitchSkillState("wzzz_v__chenglue", false) == fk.SwitchYang and "#wzzz_v__chenglue-active:::1:2" or "#wzzz_v__chenglue-active:::2:1"
  end,
  ...
  on_use = function(self, room, effect)
    ...
    player:drawCards(isYang and 1 or 2, wzzz_v__chenglue.name)
    ...
    local toDiscard = room:askToDiscard(player, {
      ...
      cancelable = false,
      prompt = "#wzzz_v__chenglue-discard:::"..num,
      skip = true,
    })
    ...
    for _, id in ipairs(toDiscard) do
      ...
      if suit ~= "log_nosuit" then
        room:addTableMarkIfNeed(player, "@wzzz_v__chenglue-phase", suit)
      end
    end
...
wzzz_v__chenglue:addEffect(fk.PreCardUse, {
  can_refresh = function(self, event, target, player, data)
    return target == player and table.contains(player:getTableMark("@wzzz_v__chenglue-phase"), data.card:getSuitString(true))
  end,
...
wzzz_v__chenglue:addEffect("targetmod", {
  bypass_times = function(self, player, skill, scope, card)
    local mark = player:getTableMark("@wzzz_v__chenglue-phase")
    if #mark > 0 then
