...
Fk:loadTranslationTable{
  ["wzzz_v__fuyin"] = "父荫",
  [":wzzz_v__fuyin"] = "锁定技，你每回合第一次成为【杀】或【决斗】的目标后，若你的手牌数不大于使用者，此牌对你无效。",

  ["$wzzz_v__fuyin1"] = "得父荫庇，平步青云。",
  ["$wzzz_v__fuyin2"] = "吾自幼心怀父诫，方不愧父亲荫庇。",
}
...
fuyin:addEffect(fk.TargetConfirmed, {
  ...
  can_trigger = function(self, event, target, player, data)
    if target == player and player:hasSkill(fuyin.name) and table.contains({"slash", "duel"}, data.card.trueName) then
      ...
      local mark = player:getMark("wzzz_v__fuyin_record-turn")
      ...
      if mark == 0 then
        room.logic:getEventsOfScope(GameEvent.UseCard, 1, function (e)
          ...
          if table.contains({"slash", "duel"}, use.card.trueName) and table.contains(use.tos, player) then
            mark = e.id
            room:setPlayerMark(player, "wzzz_v__fuyin_record-turn", mark)
            return true
          end
