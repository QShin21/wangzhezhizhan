local chenqing = fk.CreateSkill {
  name = "wzzz_v__chenqing",
}

Fk:loadTranslationTable {
  ["wzzz_v__chenqing"] = "陈情",
  [":wzzz_v__chenqing"] = "每轮限一次，当一名角色进入濒死状态时，你可以令另一名其他角色摸四张牌，然后弃置四张牌，若其以此法弃置的四张牌花色各不相同，"..
  "则其视为对濒死状态的角色使用一张【桃】。",

  ["#wzzz_v__chenqing-choose"] = "陈情：令一名角色摸四张牌然后弃四张牌，若花色各不相同视为对 %dest 使用【桃】",
  ["#wzzz_v__chenqing-discard"] = "陈情：弃置四张牌，若花色各不相同则视为对 %dest 使用【桃】",

  ["$wzzz_v__chenqing1"] = "陈生死离别之苦，悲乱世之跌宕。",
  ["$wzzz_v__chenqing2"] = "乱世陈情，字字血泪！",
}
...
chenqing:addEffect(fk.EnterDying, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local to = room:askToChoosePlayers(player, {
      ...
      max_num = 1,
      targets = targets,
      prompt = "#wzzz_v__chenqing-choose::"..target.id,
      cancelable = true,
    })
    if #to > 0 then
  ...
  on_use = function(self, event, target, player, data)
    ...
    local cards = room:askToDiscard(to, {
      ...
      skill_name = chenqing.name,
      cancelable = false,
      prompt = "#wzzz_v__chenqing-discard::"..target.id,
      skip = true,
    })
    local suits = {}
