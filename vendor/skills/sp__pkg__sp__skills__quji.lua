local wzzz_v__quji = fk.CreateSkill {
  name = "wzzz_v__quji",
}
...
Fk:loadTranslationTable{
  ...
  ["$wzzz_v__quji1"] = "若不去兵之疾，则将何以守国？",
  ["$wzzz_v__quji2"] = "愿为将士，略尽绵薄。",
}

wzzz_v__quji:addEffect("active", {
  anim_type = "support",
  ...
  prompt = function(self, player)
    return "#wzzz_v__quji:::"..player:getLostHp()
  end,
  can_use = function(self, player)
    return player:isWounded() and player:usedSkillTimes(wzzz_v__quji.name, Player.HistoryPhase) == 0
  end,
  ...
  on_use = function(self, room, effect)
    ...
    room:throwCard(effect.cards, wzzz_v__quji.name, player, player)
    ...
    if loseHp and not player.dead then
      room:loseHp(player, 1, wzzz_v__quji.name)
    end
  end,
})
