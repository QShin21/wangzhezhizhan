...
Fk:loadTranslationTable{
  ["wzzz_v__lihun"] = "离魂",
  [":wzzz_v__lihun"] = "出牌阶段限一次，你可以弃置一张牌并将你的武将牌翻面，若如此做，指定一名男性角色，获得其所有手牌。出牌阶段结束时，"..
  "你须为该角色的每一点体力分配给其一张牌。",

  ["#wzzz_v__lihun"] = "离魂：弃一张牌并翻面，获得一名角色所有手牌，本阶段结束时交给其体力值的牌",
  ["#wzzz_v__lihun-give"] = "离魂：你需交还 %dest %arg张牌",

  ["$wzzz_v__lihun1"] = "将军~这些都赏给妾身好不好嘛~",
  ["$wzzz_v__lihun2"] = "这些人家不喜欢嘛~还给你吧~",
}

lihun:addEffect("active", {
  mute = true,
  prompt = "#wzzz_v__lihun",
  card_num = 1,
  target_num = 1,
  ...
  on_use = function(self, room, effect)
    ...
    room:addTableMarkIfNeed(player, "wzzz_v__lihun-phase", target.id)
...
lihun:addEffect(fk.EventPhaseEnd, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player.phase == Player.Play and player:getMark("wzzz_v__lihun-phase") ~= 0
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    player:broadcastSkillInvoke(lihun.name, 2)
    room:notifySkillInvoked(player, lihun.name, "control")
    local targets = table.map(player:getMark("wzzz_v__lihun-phase"), Util.Id2PlayerMapper)
      ...
      if not to.dead then
        ...
        if n < #cards then
          cards = room:askToCards(player, {
            ...
            cancelable = false,
            prompt = "#wzzz_v__lihun-give::"..to.id..":"..n,
          })
        end
