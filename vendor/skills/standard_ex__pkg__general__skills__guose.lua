Fk:loadTranslationTable{
  ...
  ["$wzzz_v__ex__guose1"] = "旅途劳顿，请下马休整吧~",
  ["$wzzz_v__ex__guose2"] = "还没到休息的时候！",
}

local guose = fk.CreateSkill{
  name = "wzzz_v__ex__guose",
}
...
guose:addEffect("active", {
  ...
  interaction = UI.ComboBox {choices = {"wzzz_v__ex__guose_use" , "wzzz_v__ex__guose_throw"}},
  card_filter = function(self, player, to_select, selected)
    if #selected > 0 or not self.interaction.data or Fk:getCardById(to_select).suit ~= Card.Diamond then return false end
    if self.interaction.data == "wzzz_v__ex__guose_use" then
