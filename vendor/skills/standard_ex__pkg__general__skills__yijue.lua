Fk:loadTranslationTable{
  ...
  [":wzzz_v__ex__yijue"] = "出牌阶段限一次，你可以弃置一张牌，然后令一名其他角色展示一张手牌。若此牌为："..
    "黑色，其本回合非锁定技失效且不能使用或打出手牌，你本回合对其使用的<font color='red'>♥</font>【杀】伤害+1；"..
    "红色，你获得之，然后你可以令其回复1点体力。",
  ["#wzzz_v__ex__yijue"] = "义绝：弃置一张牌，令一名角色展示一张手牌",
  ["@@wzzz_v__ex__yijue-turn"] = "义绝",
  ["#wzzz_v__ex__yijue-show"] = "义绝：请展示一张手牌",
  ["#wzzz_v__ex__yijue-recover"] = "义绝：是否令%dest回复1点体力？",

  ["$wzzz_v__ex__yijue1"] = "关某，向来恩怨分明！",
  ["$wzzz_v__ex__yijue2"] = "恩已断，义当绝！",
}

local yijue = fk.CreateSkill{
  name = "wzzz_v__ex__yijue",
}
    ...
    if Fk:getCardById(card).color == Card.Black then
      ...
      if not player.dead and not to.dead and to:isWounded() and
        room:askToSkillInvoke(player, {
          skill_name = yijue.name,
          prompt = "#wzzz_v__ex__yijue-recover::"..to.id,
        }) then
        room:recover{
          who = to,
          num = 1,
...
yijue:addEffect("prohibit", {
  prohibit_use = function(self, player, card)
    if player:getMark("@@wzzz_v__ex__yijue-turn") ~= 0 then
...
yijue:addEffect(fk.DamageCaused, {
  ...
  can_trigger = function(self, event, target, player, data)
    return
      ...
      type(data.to:getMark("@@wzzz_v__ex__yijue-turn")) == "table" and
      data.to:getMark("@@wzzz_v__ex__yijue-turn")[tostring(player.id)] and
      player.room.logic:damageByCardEffect()
  end,
  on_use = function(self, event, target, player, data)
    data:changeDamage(data.to:getMark("@@wzzz_v__ex__yijue-turn")[tostring(player.id)])
  end,
})
