...
danshou:addEffect(fk.TargetConfirmed, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      ...
      prompt = "#wzzz_v__ty_ex__danshou-draw:::"..n,
    }) then
  ...
  on_use = function(self, event, target, player, data)
    player.room:setPlayerMark(player, "@wzzz_v__ty_ex__danshou-turn", 0)
  ...
  on_refresh = function(self, event, target, player, data)
    ...
    room:setPlayerMark(player, "@wzzz_v__ty_ex__danshou-turn", n)
  end,
})
...
danshou:addEffect(fk.EventPhaseStart, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if n == 0 then
      if room:askToSkillInvoke(player, {
        skill_name = danshou.name,
        prompt = "#wzzz_v__ty_ex__danshou-invoke::"..target.id,
      }) then
