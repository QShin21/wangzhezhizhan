local longyin = fk.CreateSkill {
  name = "wzzz_v__ty_ex__longyin",
}

Fk:loadTranslationTable{
  ["wzzz_v__ty_ex__longyin"] = "龙吟",
  [":wzzz_v__ty_ex__longyin"] = "当一名角色于其出牌阶段使用【杀】时，你可以弃置一张牌令此【杀】不计入出牌阶段使用次数。若此【杀】为红色，"..
  "你摸一张牌；若你弃置的牌点数与此【杀】相同，你重置〖竭忠〗。",

  ["#wzzz_v__ty_ex__longyin-invoke"] = "龙吟：你可以弃置一张牌，令 %dest 的【杀】不计入次数限制",

  ["$wzzz_v__ty_ex__longyin1"] = "风云将起，龙虎齐鸣！",
  ["$wzzz_v__ty_ex__longyin2"] = "武圣龙威，破敌无惧！"
}
...
longyin:addEffect(fk.CardUsing, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local card = room:askToDiscard(player, {
      ...
      cancelable = true,
      prompt = "#wzzz_v__ty_ex__longyin-invoke::" .. target.id,
      skip = true,
    })
    if #card > 0 then
  ...
  on_use = function(self, event, target, player, data)
    ...
    if data.card.number == Fk:getCardById(cards[1]).number then
      player:setSkillUseHistory("wzzz_v__ty_ex__jiezhong", 0, Player.HistoryGame)
    end
  end,
})
