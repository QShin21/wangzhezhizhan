local shenxing = fk.CreateSkill {
  name = "wzzz_v__ty_ex__shenxing",
}

Fk:loadTranslationTable{
  ["wzzz_v__ty_ex__shenxing"] = "慎行",
  [":wzzz_v__ty_ex__shenxing"] = "出牌阶段，你可以弃置X张牌，然后摸一张牌（X为你此阶段发动本技能次数，至多为2）。",

  ["#wzzz_v__ty_ex__shenxing-draw"] = "慎行：你可以摸一张牌",
  ["#wzzz_v__ty_ex__shenxing"] = "慎行：你可以弃置%arg张牌，摸一张牌",

  ["$wzzz_v__ty_ex__shenxing1"] = "谋而后动，行不容差。",
  ["$wzzz_v__ty_ex__shenxing2"] = "谋略之道，需慎之又慎。",
}
...
shenxing:addEffect("active", {
  ...
  prompt = function(self, player)
    ...
    if n == 0 then
      return "#wzzz_v__ty_ex__shenxing-draw"
    else
      return "#wzzz_v__ty_ex__shenxing:::"..math.min(2, n)
    end
  end,
