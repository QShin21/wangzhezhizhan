...
Fk:loadTranslationTable{
  ...
  [":wzzz_v__ty_ex__xianzhen"] = "每回合限一次，出牌阶段，你可以与一名角色拼点：若你赢，本回合你无视该角色的防具、对其使用牌无距离和次数限制，"..
...
xianzhen:addEffect("active", {
  ...
  times = function (_, player)
    return 1 - player:usedSkillTimes(xianzhen.name, Player.HistoryTurn)
  end,
...
xianzhen:addEffect("maxcards", {
  exclude_from = function(self, player, card)
    return card.trueName == "slash" and player:getMark("wzzz_v__ty_ex__xianzhen_lose-turn") > 0
  end,
})
