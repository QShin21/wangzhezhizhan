local choutao = fk.CreateSkill {
  name = "wzzz_v__choutao",
}

Fk:loadTranslationTable{
  ["wzzz_v__choutao"] = "仇讨",
  [":wzzz_v__choutao"] = "当你使用【杀】指定目标后，或当你成为【杀】的目标后，你可以弃置此【杀】使用者的一张牌，令此【杀】不能被抵消。"..
  "若使用者为你，此【杀】不计入次数。",

  ["#wzzz_v__choutao_self-invoke"] = "仇讨：你可以弃置一张牌，令此【杀】不能被响应且不计次数",
  ["#wzzz_v__choutao-invoke"] = "仇讨：你可以弃置 %dest 一张牌令此【杀】不能被响应",

  ["$wzzz_v__choutao1"] = "大恨深仇，此剑讨之！",
  ["$wzzz_v__choutao2"] = "是非恩怨，此役决之！"
}
...
choutao:addEffect(fk.TargetSpecified, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local card = room:askToDiscard(player, {
      ...
      prompt = "#wzzz_v__choutao_self-invoke",
      cancelable = true,
      skip = true,
    })
    if #card > 0 then
...
choutao:addEffect(fk.TargetConfirmed, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      skill_name = choutao.name,
      prompt = "#wzzz_v__choutao-invoke::" .. data.from.id,
    }) then
