...
Fk:loadTranslationTable{
  ...
  ["#wzzz_v__ty__mouzhu-choice"] = "谋诛：视为对 %dest 使用【杀】或【决斗】！",

  ["$wzzz_v__ty__mouzhu1"] = "尔等祸乱朝纲，罪无可赦，按律当诛！",
  ["$wzzz_v__ty__mouzhu2"] = "天下人之怨皆系于汝等，还不快认罪伏法？",
}

mouzhu:addEffect("active", {
  anim_type = "control",
  prompt = "#wzzz_v__ty__mouzhu",
  card_num = 0,
  min_target_num = 1,
    ...
    for _, target in ipairs(effect.tos) do
      if player.dead then return end
        ...
        local card = room:askToCards(target, {
          ...
          cancelable = false,
          prompt = "#wzzz_v__ty__mouzhu-give::"..player.id,
        })
        ...
        if not target.dead and player:getHandcardNum() > target:getHandcardNum() then
          ...
          if #names > 0 then
            local choice = room:askToChoice(target, {
              choices = names,
              skill_name = mouzhu.name,
              prompt = "#wzzz_v__ty__mouzhu-choice::"..player.id,
            })
