local zhenlie = fk.CreateSkill {
  name = "wzzz_v__zhenlie",
}

Fk:loadTranslationTable{
  ["wzzz_v__zhenlie"] = "贞烈",
  [":wzzz_v__zhenlie"] = "当你成为其他角色使用【杀】或普通锦囊牌的目标后，你可以失去1点体力使此牌对你无效，然后你弃置其一张牌。",

  ["#wzzz_v__zhenlie-invoke"] = "贞烈：是否失去1点体力，令 %dest 使用的%arg对你无效？",

  ["$wzzz_v__zhenlie1"] = "虽是妇人，亦当奋身一搏！",
  ["$wzzz_v__zhenlie2"] = "为雪前耻，不惜吾身！",
}
...
zhenlie:addEffect(fk.TargetConfirmed, {
  ...
  on_cost = function (self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      ...
      prompt = "#wzzz_v__zhenlie-invoke::"..data.from.id..":"..data.card:toLogString(),
    }) then
