...
Fk:loadTranslationTable{
  ...
  ["#wzzz_v__fencheng-discard"] = "焚城：弃置至少%arg张牌，否则受到2点火焰伤害",

  ["$wzzz_v__fencheng1"] = "我得不到的，你们也别想得到！",
  ["$wzzz_v__fencheng2"] = "让这一切都灰飞烟灭吧！哼哼哼哼……",
}

fencheng:addEffect("active", {
  anim_type = "offensive",
  prompt = "#wzzz_v__fencheng",
  card_num = 0,
  target_num = 0,
    ...
    for _, target in ipairs(targets) do
      if not target.dead then
        local cards = room:askToDiscard(target, {
          ...
          cancelable = true,
          prompt = "#wzzz_v__fencheng-discard:::"..(n + 1),
        })
        if #cards == 0 then
          room:damage{
            from = player,
