...
Fk:loadTranslationTable{
  ["wzzz_v__renxin"] = "仁心",
  [":wzzz_v__renxin"] = "当体力值为1的其他角色受到伤害时，你可以弃置一张装备牌，将武将牌翻面并防止此伤害。",

  ["#wzzz_v__renxin-invoke"] = "仁心：你可以弃置一张装备牌并翻面，防止 %dest 受到的致命伤害",

  ["$wzzz_v__renxin1"] = "仁者爱人，人恒爱之。",
  ["$wzzz_v__renxin2"] = "有我在，别怕。",
}
...
renxin:addEffect(fk.DetermineDamageInflicted, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local card = room:askToDiscard(player, {
      ...
      cancelable = true,
      pattern = ".|.|.|.|.|equip",
      prompt = "#wzzz_v__renxin-invoke::" .. target.id,
      skip = true,
    })
    if #card > 0 then
