...
local qiangzhi = fk.CreateSkill {
  name = "wzzz_v__qiangzhi",
}

Fk:loadTranslationTable{
  ["wzzz_v__qiangzhi"] = "强识",
  [":wzzz_v__qiangzhi"] = "出牌阶段开始时，你可以展示一名其他角色的一张手牌，当你于此阶段内使用与此牌类别相同的牌时，你可以摸一张牌。",

  ["#wzzz_v__qiangzhi-choose"] = "强识：展示一名其他角色的一张手牌，此阶段内你使用类别相同的牌时，你可以摸一张牌",
  ["@wzzz_v__qiangzhi-phase"] = "强识",
  ["#wzzz_v__qiangzhi-invoke"] = "强识：你可以摸一张牌",

  ["$wzzz_v__qiangzhi1"] = "容我过目，即刻咏来。",
  ["$wzzz_v__qiangzhi2"] = "文书强识，才可博于运筹。",
}
...
qiangzhi:addEffect(fk.EventPhaseStart, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local to = room:askToChoosePlayers(player, {
      ...
      max_num = 1,
      targets = targets,
      prompt = "#wzzz_v__qiangzhi-choose",
      cancelable = true,
    })
    if #to > 0 then
  ...
  on_use = function(self, event, target, player, data)
    ...
    if not player.dead then
      room:setPlayerMark(player, "@wzzz_v__qiangzhi-phase", type)
    end
  end,
})
...
qiangzhi:addEffect(fk.CardUsing, {
  ...
  return target == player and player.phase == Player.Play and
    data.card:getTypeString() == player:getMark("@wzzz_v__qiangzhi-phase")
  end,
  on_cost = function(self, event, target, player, data)
    return player.room:askToSkillInvoke(player, {
      skill_name = qiangzhi.name,
      prompt = "#wzzz_v__qiangzhi-invoke",
    })
  end,
  on_use = function(self, event, target, player, data)
  local room = player.room
    player:broadcastSkillInvoke("wzzz_v__qiangzhi", 2)
