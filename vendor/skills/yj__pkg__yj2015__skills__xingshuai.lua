local xingshuai = fk.CreateSkill {
  name = "wzzz_v__xingshuai",
  tags = { Skill.Lord, Skill.Limited },
}

Fk:loadTranslationTable{
  ["wzzz_v__xingshuai"] = "兴衰",
  [":wzzz_v__xingshuai"] = "主公技，限定技，当你进入濒死状态时，你可令其他魏势力角色依次选择是否令你回复1点体力。选择是的角色在此次濒死结算结束后"..
  "受到1点无来源的伤害。",

  ["#wzzz_v__xingshuai-invoke"] = "兴衰：其他魏势力角色可以受到1点伤害，令你回复1点体力",
  ["#wzzz_v__xingshuai-ask"] = "兴衰：你可以令 %dest 回复1点体力，结算后你受到1点伤害",

  ["$wzzz_v__xingshuai1"] = "百年兴衰皆由人，不由天！",
  ["$wzzz_v__xingshuai2"] = "聚群臣而嘉勋，隆天子之气运！",
}
...
xingshuai:addEffect(fk.EnterDying, {
  ...
  on_cost = function (self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      ...
      prompt = "#wzzz_v__xingshuai-invoke",
    }) then
    ...
    for _, p in ipairs(room:getOtherPlayers(player)) do
      if p.kingdom == "wei" and
        room:askToSkillInvoke(p, {
          skill_name = xingshuai.name,
          prompt = "#wzzz_v__xingshuai-ask::"..player.id
        }) then
