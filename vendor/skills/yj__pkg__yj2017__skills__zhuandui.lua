local zhuandui = fk.CreateSkill {
  name = "wzzz_v__zhuandui"
}

Fk:loadTranslationTable{
  ["wzzz_v__zhuandui"] = "专对",
  [":wzzz_v__zhuandui"] = "当你使用【杀】指定目标后，你可以与目标拼点：若你赢，其不能响应此【杀】。当你成为【杀】的目标后，你可以与使用者拼点："..
  "若你赢，此【杀】对你无效。",

  ["wzzz_v__zhuandui1-invoke"] = "专对：你可以与 %dest 点，若赢，其不能响应此%arg",
  ["wzzz_v__zhuandui2-invoke"] = "专对：你可以与 %dest 拼点，若赢，此%arg对你无效",

  ["$wzzz_v__zhuandui1"] = "黄口小儿，也敢来班门弄斧？",
  ["$wzzz_v__zhuandui2"] = "你已无话可说了吧！",
}
...
zhuandui:addEffect(fk.TargetSpecified, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      ...
      prompt = "wzzz_v__zhuandui1-invoke::"..data.to.id..":"..data.card:toLogString(),
    }) then
...
zhuandui:addEffect(fk.TargetConfirmed, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      skill_name = zhuandui.name,
      prompt = "wzzz_v__zhuandui2-invoke::"..data.from.id..":"..data.card:toLogString(),
    }) then
